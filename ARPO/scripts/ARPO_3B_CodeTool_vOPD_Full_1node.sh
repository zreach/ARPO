...
# vOPD full-V from "KL for a KL": base OPD reward plus the detached full-vocab
# reverse-KL control variate baseline. This is exact but materially heavier than top-k.
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-vopd_full_3b_code_tool_only}"

export USE_OPD_LOSS="${USE_OPD_LOSS:-False}"
export OPD_LOSS_COEF="${OPD_LOSS_COEF:-0.0}"

# vopd_topk <= 0 means use the whole vocabulary.
VOPD_TOPK="${VOPD_TOPK:-0}"
